...
public:
  ...
  */
  ...
  sp_rcontext *get_rcontext(const sp_rcontext_addr &addr) const;
  Item_field *get_variable(const sp_rcontext_addr &addr) const;

  // Get a static CURSOR or a one referenced by SYS_REFCURSOR
  sp_cursor *get_cursor(const sp_rcontext_addr &addr,
                        const Sp_rcontext_handler *pointer);
  sp_cursor *get_open_cursor_or_error(const sp_rcontext_addr &addr,
                                      const Sp_rcontext_handler *pointer);
