...
int QUICK_GROUP_MIN_MAX_SELECT::skip_nulls(bool reverse)
{
  ...
  */
  if (min_max_arg_part && min_max_arg_part->field->is_null())
  {
    ...
    next call to next_min_max(true, _), and to save one lookup in the
...
int QUICK_GROUP_MIN_MAX_SELECT::next_min_max_in_range(bool min, bool reverse)
{
  ...
  QUICK_RANGE *cur_range;
  int result;
  /* Whether looking for the last key (true) or first key (false). */
  bool last= min == reverse;
  ...
  */
  for (size_t range_idx= 0; range_idx < min_max_ranges.elements; range_idx++)
  {
    ...
    /*
      If the current value for the max (resp. min) argument comes
      before (resp. after) cur_range, there is no need to check this
      range.
    */
    if (range_idx != 0 &&
        ((!min &&
    ...
    /*
      If looking for max (resp. min) and the current range has no
      upper bound (resp. lower bound), set unconditional find_flag.
    */
    ...
    else
    {
      /*
        ...
        For example, if looking for min with descending index (hence
        last == true), and the current range has an open lower
        boundary (NEAR_MIN), the find_flag is HA_READ_BEFORE_KEY.
      */
    ...
    if (result)
    {
      ...
      /*
        In no key was found within this boundary, there certainly are
        no keys subsequent boundaries.
      */
      return result;
    }
    /* A key was found. */
    ...
    /*
      Compare the found max key (resp. min key) with the lower
      boundary (resp. the upper boundary) if the said boundary
      exists.
    */
         ...
         (min && !(cur_range->flag & NO_MAX_RANGE)))
    {
      ...
      /*
        ...
        - the max key (resp. min key) is less than the lower boundary
          (resp. greater than the upper boundary)
      */
