...
{
  int len= (int) snprintf(buf, size, "%s", pass);
...
static int ssl_external_passwd_cb(char *buf, int size, int rw, void *userdata)
{
  ...
  {
    {"pass:", 5, passwd_from_string},
    {"file:", 5, passwd_from_file},
    {"env:", 4, passwd_from_env}
  };

  for (size_t i= 0; i < sizeof(handlers) / sizeof(handlers[0]); i++)
  {
    const struct Handler *h= &handlers[i];
    if (strncmp(cmd, h->prefix, h->prefix_len) == 0)
    {
      int len= h->handler(buf, size, cmd + h->prefix_len);
